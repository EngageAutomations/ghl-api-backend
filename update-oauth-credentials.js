/**
 * Update OAuth Backend with Correct Credentials
 * Uses the correct client credentials from attached assets
 */

import { Octokit } from '@octokit/rest';

async function updateOAuthCredentials() {
  console.log('=== Updating OAuth Backend with Correct Credentials ===');
  ...
  try {
    ...
    console.log('2. Creating OAuth backend with correct credentials...');
    
    // OAuth backend with CORRECT credentials from Client Key file
    const correctedOAuthBackend = `/**
...
app.listen(port, () => {
  ...
  console.log('API Backend: Separate service handles GoHighLevel API calls');
    ...
    console.log('3. Deploying OAuth backend with correct credentials...');
    
    await octokit.rest.repos.createOrUpdateFileContents({
      owner,
      repo,
      path: 'index.js',
      message: 'Update OAuth backend with correct client credentials',
      content: Buffer.from(correctedOAuthBackend).toString('base64'),
      sha: currentIndex.sha,
      branch
    });
    
    console.log('✅ OAuth Backend updated with correct credentials!');
    console.log('');
    ...
    console.log('   Enhanced UI for success/error pages');
    ...
    console.log('');
    console.log('📋 Ready for OAuth Installation:');
    console.log('1. Wait for deployment (2-3 minutes)');
    console.log('2. Try OAuth installation again');
    console.log('3. Should now succeed with correct credentials');
