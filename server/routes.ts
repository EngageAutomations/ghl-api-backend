...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // New GoHighLevel API endpoints using the enhanced service
  ...
  app.post("/api/ghl/products/create", async (req, res) => {
    try {
      ...
      console.error('Error creating GHL product:', error);
      res.status(500).json({ 
  ...
  app.get("/api/ghl/products/list", async (req, res) => {
    try {
  ...
  app.put("/api/ghl/products/:productId", async (req, res) => {
    try {
      ...
      res.json({ 
        success: true, 
        product: result,
        message: 'Product updated successfully in GoHighLevel'
      });
    } catch (error) {
      console.error('Error updating GHL product:', error);
      res.status(500).json({ 
  ...
  // Delete product from GoHighLevel
  app.delete("/api/ghl/products/:productId", async (req, res) => {
    try {
      ...
      res.json({ 
        success: true,
        message: 'Product deleted successfully from GoHighLevel'
      });
    } catch (error) {
  ...
  // Legacy GoHighLevel Product Creation API (kept for compatibility)
