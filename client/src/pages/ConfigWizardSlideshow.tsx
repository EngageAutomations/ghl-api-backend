  ...
  const generateCodeForSelection = () => {
    if (formEmbedUrl && formEmbedUrl.trim()) {
      if (buttonType === 'popup') {
        ...
        // Generate Cart Page CSS if needed
        const cartPageCode = showCartCustomization ? cartPageCssCode : '';
        
        // Generate Cart Icon CSS if needed
        const cartIconCode = showCartCustomization ? cartIconCssCode : '';
        
        // Generate Price Removal CSS if needed
        const priceRemovalCode = showPriceRemoval ? priceRemovalCssCode : '';

        return {
          css: `${expandedDescCode.css}\n${metadataCode.css}\n${cartPageCode}\n${cartIconCode}\n${priceRemovalCode}`.trim(),
          headerCode: popupCode.headerCode + expandedDescCode.headerCode + metadataCode.headerCode,
          footerCode: popupCode.footerCode + expandedDescCode.footerCode + metadataCode.footerCode
        };
      } else if (buttonType === 'embed') {
        // Generate embedded form code
        const embeddedCode = generateEmbeddedFormCode({
          formUrl: extractFormUrl(formEmbedUrl),
          customFieldName: customFieldName || 'listing'
        });
        ...
        return {
          css: `${embeddedCode.css}\n${expandedDescCode.css}\n${metadataCode.css}`.trim(),
          headerCode: embeddedCode.headerCode + expandedDescCode.headerCode + metadataCode.headerCode,
          footerCode: embeddedCode.footerCode + expandedDescCode.footerCode + metadataCode.footerCode
        };
      }
    }

    return { css: '', headerCode: '', footerCode: '' };
  };
            ...
            >
              <div className="flex items-start space-x-3 w-full min-w-0">
                ...
                <div className="text-left min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500 leading-tight mt-1">Shows form directly on page</p>
                </div>
              </div>
            </Button>
            </div>
